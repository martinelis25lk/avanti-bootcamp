...
export class EditarItemDto {
  ...
  validar() {
    ...
    if (this.data_entregue) {
      if (typeof this.data_entregue !== "string") {
        erros.push({
          campo: "data_entregue",
          mensagem: MENSAGEM_DTO.ITEM.DATA_ENTREGUE.STRING,
        });
      } else if (!validarData(this.data_entregue)) {
        erros.push({
          campo: "data_entregue",
          mensagem: MENSAGEM_DTO.ITEM.DATA_ENTREGUE.INVALIDO,
        });
      }
    }
